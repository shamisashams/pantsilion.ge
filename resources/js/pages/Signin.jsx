...
const Signin = ({seo}) => {
    ...
    return <Layout seo={seo}><div className="bg-zinc-100">
        <div className="wrapper max-w-md py-52 text-center">
            <div className="sm:text-5xl text-3xl bold mb-10">{__('client.login', localizations)}</div>
            {errors.email && <div>{errors.email}</div>}
            <form onSubmit={handleSubmit}>
                <input type="text" onChange={handleChange} name="email" placeholder={__('client.form_email', localizations)} className="mb-3 border border-zinc-200 pl-4 w-full block bg-transparent h-12 placeholder:text-custom-dark" />
                <input type="password" onChange={handleChange} name="password" placeholder={__('client.form_password', localizations)} className="mb-7 border border-zinc-200 pl-4 w-full block bg-transparent h-12 placeholder:text-custom-dark" />
            ...
            <p className="my-6">{__('client.login_not_account', localizations)}  <Link href={route('client.registration.index')} className="text-sky-600 underline"> {__('client.login_signup', localizations)}</Link></p>
        </div>
    </div></Layout>
}
